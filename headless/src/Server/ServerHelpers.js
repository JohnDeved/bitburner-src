...
exports.ServerOwnershipType = void 0;
exports.safelyCreateUniqueServer = safelyCreateUniqueServer;
exports.numCycleForGrowth = numCycleForGrowth;
exports.numCycleForGrowthCorrected = numCycleForGrowthCorrected;
...
exports.isBackdoorInstalledInCompanyServer = isBackdoorInstalledInCompanyServer;
exports.getCoreBonus = getCoreBonus;
exports.getWeakenEffect = getWeakenEffect;
exports.checkServerOwnership = checkServerOwnership;
const AllServers_1 = require("./AllServers");
const Server_1 = require("./Server");
const grow_1 = require("./formulas/grow");
...
var ServerOwnershipType;
(function (ServerOwnershipType) {
    ServerOwnershipType[ServerOwnershipType["All"] = 0] = "All";
    ServerOwnershipType[ServerOwnershipType["Foreign"] = 1] = "Foreign";
    ServerOwnershipType[ServerOwnershipType["Owned"] = 2] = "Owned";
    ServerOwnershipType[ServerOwnershipType["Purchased"] = 3] = "Purchased";
})(ServerOwnershipType || (exports.ServerOwnershipType = ServerOwnershipType = {}));
/**
 * Constructs a new server, while also ensuring that the new server
 * does not have a duplicate hostname/ip.
 */
function safelyCreateUniqueServer(params) {
    let hostname = params.hostname.replace(/ /g, `-`);
    if (params.ip != null && (0, AllServers_1.ipExists)(params.ip)) {
        params.ip = (0, AllServers_1.createUniqueRandomIp)();
    }
    if ((0, AllServers_1.GetServer)(hostname) != null) {
 ...
 */
function numCycleForGrowth(server, growth, cores = 1) {
    if (!server.serverGrowth)
        return Infinity;
    return Math.log(growth) / (0, grow_1.calculateServerGrowthLog)(server, 1, _player_1.Player, cores);
}
/**
 ...
 */
function numCycleForGrowthCorrected(server, targetMoney, startMoney, cores = 1, person = _player_1.Player) {
    if (!server.serverGrowth)
        return Infinity;
    const moneyMax = server.moneyMax ?? 1;
    if (startMoney < 0)
        startMoney = 0; // servers "can't" have less than 0 dollars on them
    if (targetMoney > moneyMax)
        targetMoney = moneyMax; // can't grow a server to more than its moneyMax
    if (targetMoney <= startMoney)
        return 0; // no growth --> no threads
    const k = (0, grow_1.calculateServerGrowthLog)(server, 1, person, cores);
...
function processSingleServerGrowth(server, threads, cores = 1) {
    const oldMoneyAvailable = server.moneyAvailable;
    server.moneyAvailable = (0, grow_1.calculateGrowMoney)(server, threads, _player_1.Player, cores);
    ...
    if (oldMoneyAvailable !== server.moneyAvailable) {
        ...
        server.fortify(2 * Constants_1.ServerConstants.ServerFortifyAmount * usedCycles);
    }
...
function prestigeHomeComputer(homeComp) {
    const hasBitflume = homeComp.programs.includes(_enums_1.CompletedProgramName.bitFlume);
    homeComp.programs.length = 0; //Remove programs
    homeComp.serversOnNetwork = [];
    homeComp.isConnectedTo = true;
    homeComp.ramUsed = 0;
    homeComp.pushProgram(_enums_1.CompletedProgramName.nuke);
    if (hasBitflume) {
        homeComp.pushProgram(_enums_1.CompletedProgramName.bitFlume);
    }
    homeComp.messages.length = 0; //Remove .lit and .msg files
    homeComp.messages.push(_enums_1.LiteratureName.HackersStartingHandbook);
    if (homeComp.runningScriptMap.size !== 0) {
        // Temporary verbose logging section to gather data on a bug
        (0, exceptionAlert_1.exceptionAlert)(new Error(`Some runningScripts were still present on home during prestige. runningScripts: ${Array.from(homeComp.runningScriptMap.keys())}`), true);
        for (const [scriptKey, byPidMap] of homeComp.runningScriptMap) {
            ...
            for (const pid of byPidMap.keys()) {
                if (WorkerScripts_1.workerScripts.has(pid))
...
function getServerOnNetwork(server, i) {
    ...
    return (0, AllServers_1.GetServer)(server.serversOnNetwork[i]);
}
...
function isBackdoorInstalledInCompanyServer(companyName) {
    const serverMeta = servers_1.serverMetadata.find((s) => s.specialName === companyName);
...
function getCoreBonus(cores = 1) {
    const coreBonus = 1 + (cores - 1) / 16;
    return coreBonus;
}
function getWeakenEffect(threads, cores) {
    const coreBonus = getCoreBonus(cores);
    return Constants_1.ServerConstants.ServerWeakenAmount * threads * coreBonus * BitNodeMultipliers_1.currentNodeMults.ServerWeakenRate;
}
function checkServerOwnership(baseServer, serverType) {
    /**
     ...
     */
    const isOwnedServer = (baseServer instanceof Server_1.Server && baseServer.purchasedByPlayer) || baseServer instanceof HacknetServer_1.HacknetServer;
    switch (serverType) {
        ...
        case ServerOwnershipType.Foreign:
            ...
            return (_player_1.Player.hasAugmentation(_enums_1.AugmentationName.TheRedPill, true) || baseServer.hostname !== SpecialServers_1.SpecialServers.WorldDaemon);
        case ServerOwnershipType.Owned:
            return isOwnedServer;
        case ServerOwnershipType.Purchased:
            return isOwnedServer && baseServer.hostname !== SpecialServers_1.SpecialServers.Home;
        default:
            (0, throwIfReachable_1.throwIfReachable)(serverType);
    }
    return false;
}
