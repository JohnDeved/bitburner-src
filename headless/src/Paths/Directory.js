...
exports.directoryRegexString = exports.oneValidCharacter = exports.root = void 0;
exports.isDirectoryPath = isDirectoryPath;
exports.isAbsolutePath = isAbsolutePath;
exports.resolveDirectory = resolveDirectory;
exports.resolveValidatedDirectory = resolveValidatedDirectory;
exports.directoryExistsOnServer = directoryExistsOnServer;
exports.getFirstDirectoryInPath = getFirstDirectoryInPath;
...
exports.oneValidCharacter = `[^${(0, lodash_1.escapeRegExp)(invalidCharacters.join(""))}\\s]`;
/** Regex string for matching the directory part of a valid filepath */
exports.directoryRegexString = `^(?<directory>(?:${exports.oneValidCharacter}+\\/)*)`;
/** Actual RegExp for validating that an entire string is a BasicDirectory */
const basicDirectoryRegex = new RegExp(exports.directoryRegexString + "$");
...
function resolveDirectory(path, base = exports.root) {
...
function directoryExistsOnServer(directory, server) {
    for (const scriptFilePath of server.scripts.keys())
        if (scriptFilePath.startsWith(directory))
            return true;
    for (const textFilePath of server.textFiles.keys())
        if (textFilePath.startsWith(directory))
            return true;
    return false;
}
...
function getAllDirectories(server) {
    const dirSet = new Set([exports.root]);
    function peel(path) {
        const lastSlashIndex = path.lastIndexOf("/", path.length - 2);
        if (lastSlashIndex === -1)
            return;
        const newDir = path.substring(0, lastSlashIndex + 1);
        if (dirSet.has(newDir))
            return;
        dirSet.add(newDir);
        peel(newDir);
    }
    for (const [filename] of (0, ContentFile_1.allContentFiles)(server))
        peel(filename);
    return dirSet;
}
// This is to validate the assertion earlier that root is in fact a Directory
if (!isDirectoryPath(exports.root) || !isAbsolutePath(exports.root))
