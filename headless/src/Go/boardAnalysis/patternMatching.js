...
exports.threeByThreePatterns = void 0;
exports.findAnyMatchedPatterns = findAnyMatchedPatterns;
const _enums_1 = require("@enums");
const boardAnalysis_1 = require("./boardAnalysis");
const goAI_1 = require("./goAI");
exports.threeByThreePatterns = [
 ...
 */
async function findAnyMatchedPatterns(board, player, availableSpaces, smart = true, rng) {
    ...
    for (let x = 0; x < boardSize; x++) {
        for (let y = 0; y < boardSize; y++) {
            ...
            if (matchedPattern &&
                ...
                (!smart || (0, boardAnalysis_1.findEffectiveLibertiesOfNewMove)(board, x, y, player).length > 1)) {
                moves.push(board[x][y]);
            }
        }
        await (0, goAI_1.sleep)(10);
    }
 ...
 */
function checkMatch(neighborhood, pattern, player) {
 ...
 */
function getNeighborhood(board, x, y) {
    return [
 ...
 */
function matches(stringPoint, point, player) {
    ...
    switch (stringPoint) {
        ...
        case ".": {
            return point?.color === _enums_1.GoColor.empty;
        }
        case " ": {
            return point === null;
        }
...
function expandAllThreeByThreePatterns() {
    const rotatedPatterns = [
        ...exports.threeByThreePatterns,
        ...exports.threeByThreePatterns.map(rotate90Degrees),
        ...exports.threeByThreePatterns.map(rotate90Degrees).map(rotate90Degrees),
        ...exports.threeByThreePatterns.map(rotate90Degrees).map(rotate90Degrees).map(rotate90Degrees),
    ];
