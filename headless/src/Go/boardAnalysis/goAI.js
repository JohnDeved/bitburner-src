...
exports.resetGoPromises = resetGoPromises;
exports.handleNextTurn = handleNextTurn;
exports.resetAI = resetAI;
exports.getMove = getMove;
exports.getExpansionMoveArray = getExpansionMoveArray;
exports.getKomi = getKomi;
exports.sleep = sleep;
exports.showWorldDemon = showWorldDemon;
const _player_1 = require("@player");
const _enums_1 = require("@enums");
const Constants_1 = require("../Constants");
const boardState_1 = require("../boardState/boardState");
const boardAnalysis_1 = require("./boardAnalysis");
const controlledTerritory_1 = require("./controlledTerritory");
...
const gameOver = { type: _enums_1.GoPlayType.gameOver, x: null, y: null };
const playerPromises = {
    [_enums_1.GoColor.black]: { nextTurn: Promise.resolve(gameOver), resolver: null },
    [_enums_1.GoColor.white]: { nextTurn: Promise.resolve(gameOver), resolver: null },
};
// The promises aren't in a fully working state until we do this.
// It is OK to reset the AI multiple times in a row.
resetAI();
function getNextTurn(color) {
    return playerPromises[color].nextTurn;
}
function resetGoPromises() {
    resetAI();
    handleNextTurn().catch((error) => (0, exceptionAlert_1.exceptionAlert)(error, true));
}
/**
 ...
 */
function handleNextTurn(boardState = Go_1.Go.currentGame, useOfflineCycles = true) {
    ...
    const currentColor = previousColor === _enums_1.GoColor.black ? _enums_1.GoColor.white : _enums_1.GoColor.black;
    ...
    const currentPromise = playerPromises[currentColor === _enums_1.GoColor.black ? _enums_1.GoColor.white : _enums_1.GoColor.black];
    ...
    if (boardState.ai !== _enums_1.GoOpponent.none && currentColor == _enums_1.GoColor.white) {
        const currentMoveCount = Go_1.Go.currentGame.previousBoards.length;
        getMove(boardState, currentColor, Go_1.Go.currentGame.ai, useOfflineCycles)
            .then(async (play) => {
            if (currentMoveCount !== Go_1.Go.currentGame.previousBoards.length || boardState !== Go_1.Go.currentGame) {
                //Stale game
                return;
            }
            // Handle AI passing
            if (play.type === _enums_1.GoPlayType.pass) {
                (0, boardState_1.passTurn)(boardState, currentColor);
            ...
            const aiUpdatedBoard = (0, boardState_1.makeMove)(boardState, play.x, play.y, currentColor);
            ...
            .catch((error) => (0, exceptionAlert_1.exceptionAlert)(error));
    }
 ...
 * the board state.
 */
function resetAI(endOfGame = false) {
...
function createPromise(promiseObj) {
    promiseObj.resolver?.();
    promiseObj.nextTurn = new Promise((resolve) => {
        promiseObj.resolver = (play) => resolve(play ?? (0, boardAnalysis_1.getPreviousMoveDetails)());
    });
}
/*
 ...
 */
async function getMove(boardState, player, opponent, useOfflineCycles = true, rngOverride) {
    await waitCycle(useOfflineCycles);
    const rng = new RNG_1.WHRNG(rngOverride || _player_1.Player.totalPlaytime);
    ...
    const moveOptions = [
        ...
        .filter((point) => (0, boardAnalysis_1.evaluateIfMoveIsValid)(boardState, point.x, point.y, player, false));
    ...
    if (chosenMove) {
        ...
        return { type: _enums_1.GoPlayType.move, x: chosenMove.x, y: chosenMove.y };
    }
    // Pass if no valid moves were found
    return { type: _enums_1.GoPlayType.pass, x: null, y: null };
}
/**
...
async function getFactionMove(moves, faction, rng) {
    if (faction === _enums_1.GoOpponent.Netburners) {
        return getNetburnersPriorityMove(moves, rng);
    }
    if (faction === _enums_1.GoOpponent.SlumSnakes) {
        return getSlumSnakesPriorityMove(moves, rng);
    }
    if (faction === _enums_1.GoOpponent.TheBlackHand) {
        return getBlackHandPriorityMove(moves, rng);
    }
    if (faction === _enums_1.GoOpponent.Tetrads) {
        return getTetradPriorityMove(moves, rng);
    }
    if (faction === _enums_1.GoOpponent.Daedalus) {
...
function findLiveNodesInArea(board, x1, y1, x2, y2) {
    const foundPoints = [];
    board.forEach((column) => column.forEach((point) => point && point.x >= x1 && point.x <= x2 && point.y >= y1 && point.y <= y2 && foundPoints.push(point)));
    return foundPoints;
}
/**
...
function isCornerAvailableForMove(board, x1, y1, x2, y2) {
    ...
    const foundPieces = foundPoints.filter((point) => point.color !== _enums_1.GoColor.empty);
 ...
 */
function getExpansionMoveArray(board, availableSpaces) {
    // Look for any empty spaces fully surrounded by empty spaces to expand into
    const emptySpaces = availableSpaces.filter((space) => {
        const neighbors = (0, boardState_1.findNeighbors)(board, space.x, space.y);
        return ([neighbors.north, neighbors.east, neighbors.south, neighbors.west].filter((point) => point && point.color === _enums_1.GoColor.empty).length === 4);
    });
...
function getDisputedTerritoryMoves(board, availableSpaces, maxChainSize = 99) {
    const chains = (0, boardAnalysis_1.getAllChains)(board).filter((chain) => chain.length <= maxChainSize);
    return availableSpaces.filter((space) => {
        const chain = chains.find((chain) => chain[0].chain === space.chain) ?? [];
        const playerNeighbors = (0, boardAnalysis_1.getAllNeighboringChains)(board, chain, chains);
        const hasWhitePieceNeighbor = playerNeighbors.find((neighborChain) => neighborChain[0]?.color === _enums_1.GoColor.white);
        const hasBlackPieceNeighbor = playerNeighbors.find((neighborChain) => neighborChain[0]?.color === _enums_1.GoColor.black);
 ...
 */
function getLibertyGrowthMoves(board, player, availableSpaces) {
    const friendlyChains = (0, boardAnalysis_1.getAllChains)(board).filter((chain) => chain[0].color === player);
    ...
    const liberties = friendlyChains
        .map((chain) => chain[0].liberties?.filter(boardState_1.isNotNullish).map((liberty) => ({
        libertyPoint: liberty,
        oldLibertyCount: chain[0].liberties?.length,
    })))
        .flat()
        .filter(boardState_1.isNotNullish)
        .filter((liberty) => availableSpaces.find((point) => liberty.libertyPoint.x === point.x && liberty.libertyPoint.y === point.y));
    ...
    return liberties
        ...
        const newLibertyCount = (0, boardAnalysis_1.findEffectiveLibertiesOfNewMove)(board, move.x, move.y, player).length;
        // Get the smallest liberty count of connected chains to represent the old state
        const oldLibertyCount = (0, boardAnalysis_1.findMinLibertyCountOfAdjacentChains)(board, move.x, move.y, player);
        return {
            point: move,
 ...
 */
function getGrowthMove(board, player, availableSpaces, rng) {
 ...
 */
function getDefendMove(board, player, availableSpaces) {
    const growthMoves = getLibertyGrowthMoves(board, player, availableSpaces);
    const libertyIncreases = growthMoves?.filter((move) => move.oldLibertyCount <= 1 && move.newLibertyCount > move.oldLibertyCount) ?? [];
 ...
 */
function getSurroundMove(board, player, availableSpaces, smart = true) {
    const opposingPlayer = player === _enums_1.GoColor.black ? _enums_1.GoColor.white : _enums_1.GoColor.black;
    const enemyChains = (0, boardAnalysis_1.getAllChains)(board).filter((chain) => chain[0].color === opposingPlayer);
    ...
    enemyLiberties.forEach((move) => {
        const newLibertyCount = (0, boardAnalysis_1.findEffectiveLibertiesOfNewMove)(board, move.x, move.y, player).length;
        const weakestEnemyChain = (0, boardAnalysis_1.findEnemyNeighborChainWithFewestLiberties)(board, move.x, move.y, player === _enums_1.GoColor.black ? _enums_1.GoColor.white : _enums_1.GoColor.black);
        ...
        const enemyLibertyGroups = [
            ...(weakestEnemyChain?.[0]?.liberties ?? []).reduce((chainIDs, point) => chainIDs.add(point?.chain ?? ""), new Set()),
        ];
 ...
 */
function getEyeCreationMoves(board, player, availableSpaces, maxLiberties = 99) {
    const allEyes = (0, boardAnalysis_1.getAllEyesByChainId)(board, player);
    const currentEyes = (0, boardAnalysis_1.getAllEyes)(board, player, allEyes);
    ...
    const friendlyLiberties = chains
        ...
        .flat()
        .filter(boardState_1.isNotNullish)
        .filter((point) => availableSpaces.find((availablePoint) => availablePoint.x === point.x && availablePoint.y === point.y))
        .filter((point) => {
        const neighbors = (0, boardState_1.findNeighbors)(board, point.x, point.y);
        const neighborhood = [neighbors.north, neighbors.east, neighbors.south, neighbors.west];
        return (neighborhood.filter((point) => !point || point?.color === player).length >= 2 &&
            neighborhood.some((point) => point?.color === _enums_1.GoColor.empty));
    });
    const eyeCreationMoves = friendlyLiberties.reduce((moveOptions, point) => {
        const evaluationBoard = (0, boardAnalysis_1.evaluateMoveResult)(board, point.x, point.y, player);
        const newEyes = (0, boardAnalysis_1.getAllEyes)(evaluationBoard, player);
        const newLivingGroupsCount = newEyes.filter((eye) => eye.length >= 2).length;
        const newEyeCount = newEyes.filter((eye) => eye.length).length;
        if (newLivingGroupsCount > currentLivingGroupsCount ||
...
function getEyeBlockingMove(board, player, availablePoints) {
    const opposingPlayer = player === _enums_1.GoColor.white ? _enums_1.GoColor.black : _enums_1.GoColor.white;
...
function getMoveOptions(boardState, player, rng, smart = true) {
    ...
    const availableSpaces = (0, controlledTerritory_1.findDisputedTerritory)(boardState, player, smart);
    ...
    const moveOptionGetters = {
        ...
        pattern: async () => {
            const point = endGameAvailable ? null : await (0, patternMatching_1.findAnyMatchedPatterns)(board, player, availableSpaces, smart, rng);
 ...
 */
function getKomi(state) {
    if (state.komiOverride !== null) {
        return state.komiOverride;
    }
    return Constants_1.opponentDetails[state.ai].komi;
}
/**
 * Allows time to pass
 */
 ...
 */
function waitCycle(useOfflineCycles = true) {
    if (useOfflineCycles && Go_1.Go.storedCycles > 0) {
        Go_1.Go.storedCycles -= 2;
        return sleep(40);
    }
    return sleep(200);
}
function showWorldDemon() {
    return _player_1.Player.hasAugmentation(_enums_1.AugmentationName.TheRedPill, true) && _player_1.Player.activeSourceFileLvl(1);
}
