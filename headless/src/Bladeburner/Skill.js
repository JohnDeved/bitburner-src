...
class Skill {
    constructor(params) {
        ...
        for (const [multName, mult] of (0, Record_1.getRecordEntries)(params.mults))
            this.mults[multName] = mult;
    }
    calculateCost(currentLevel, count = 1) {
        ...
        return Math.round(actualCount *
            BitNodeMultipliers_1.currentNodeMults.BladeburnerSkillCost *
            (this.baseCost + this.costInc * (currentLevel + (actualCount - 1) / 2)));
    }
    calculateMaxUpgradeCount(currentLevel, cost) {
        /**
         * Define:
         * - x = count
        ...
        const delta = Math.sqrt(m * m + (2 * this.costInc * cost) / BitNodeMultipliers_1.currentNodeMults.BladeburnerSkillCost);
         ...
         */
        const costOfResultPlus1 = this.calculateCost(currentLevel, (result + 1));
        if (costOfResultPlus1 <= cost) {
            return result + 1;
        }
        const costOfResult = this.calculateCost(currentLevel, result);
        if (costOfResult <= cost) {
            return result;
        }
        return result - 1;
    }
    canUpgrade(bladeburner, count = 1) {
        ...
        if (!(0, types_1.isPositiveInteger)(actualCount)) {
