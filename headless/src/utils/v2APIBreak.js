...
const V2Modal_1 = require("./V2Modal");
const singularity = [
    "applyToCompany",
    "b1tflum3",
...
const v2APIBreak = () => {
    ...
    if (!home)
        throw new Error("'home' server was not found.");
    const rules = [
        {
        ...
        },
        {
            matchJS: /ns\.scp/g,
            matchScript: /scp/g,
            reason: "scp arguments were switch, it is now scp(files, destination, optionally_source). If you were using 2 argument (not 3) this doesn't affect you.",
            offenders: [],
        },
        {
    ...
    (0, V2Modal_1.openV2Modal)();
    for (const server of (0, AllServers_1.GetAllServers)()) {
        server.runningScriptMap = new Map();
    }
    SaveObject_1.saveObject.exportGame().catch((e) => console.error(e));
};
exports.v2APIBreak = v2APIBreak;
const formatOffenders = (offenders) => {
    const files = {};
...
const formatRules = (rules) => {
    let txt = "This file contains the list of potential API break. A pattern was used to look through all your files and note the spots where you might have a problem. Not everything here is broken.";
    for (const rule of rules) {
        if (rule.offenders.length === 0)
            continue;
...
const processScript = (rules, script) => {
    const lines = script.code.split("\n");
    for (let i = 0; i < lines.length; i++) {
        for (const rule of rules) {
            const line = lines[i];
            const match = (0, ScriptFilePath_1.isLegacyScript)(script.filename) ? rule.matchScript ?? rule.matchJS : rule.matchJS;
            if (line.match(match)) {
                rule.offenders.push({
